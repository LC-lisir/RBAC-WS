  ...
  userApi.getloginId(token).then(response=>{
    ...
    loginOrRegister(){
      ...
      if(this.dialogTitle == "注册"){
        axios.get('http://192.168.1.110:9999/register?name=' + this.loginName)
        ...
        axios.get('http://192.168.1.110:9999/login?name=' + this.loginName)
          .then(function (response) {
    ...
    loginOut(){
      let thus = this
      axios.get('http://192.168.1.110:9999/loginOut?name=' + this.curUserName)
        .then(function (response) {
          if(response.data.code == -1){
            return thus.$message.error(response.data.errDesc);
          }
